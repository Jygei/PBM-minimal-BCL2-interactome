...
srand (2);								/************************************** ATTENTION: change between srand(time(NULL)) -> mitos at random positions or srand(1) -> mitos at the same position **************************************/
...
mito_counter = 0;
	while (mito_counter < mito_number)
	{
		...
		while (intersecting == 1)
		{
			...
			// Prevent mito-creation in the nucleus/blackbox; no minimal distance needed if ligand diffuses through blackbox, otherwise add +/- diffusion_distance -> if mito is in blackbox (intersection = 1) it generates new random start coordinates
			/*if (intersecting == 0)
			{
...
while (state2_counter < number_state_2)
{
	...
	while (intersection == 0)
	{
		...
		intersection = 1;
		// check-up that Ligand is not spawning in the nucleus/blackbox
		/*if (intersection == 1)
		{
...
/* Create tBid Receptor Agents */
printf ("Creating tBid Receptor Agents\n");
