void savedatatofile(int itno, xmachine_Receptor ** pointer_to_Receptors, xmachine_Ligand ** pointer_to_Ligands, xmachine_Mito ** pointer_to_Mitos, char * excel_filepath)
{
FILE *file;
...
if (itno == 0)
{
	...
	fprintf(file, "Mcl1aBak\t");
	fprintf(file, "Bak Mito\t");
	fprintf(file, "aBak\t");
	fprintf(file, "aBak2\t");
	fprintf(file, "aBak4\t");
	fprintf(file, "aBak6\t");
	fprintf(file, "tBid Cyto\t");
	fprintf(file, "tBid Mito\t");
	fprintf(file, "tBidBak\t");
	fprintf(file, "tBidaBak\t");
	fprintf(file, "tBidMcl1\t");
...
while(current_Receptor)
{
	...
	if (current_Receptor->state == 20)	{	State_20++;		}	//Mcl1Mito
	if (current_Receptor->state == 21)	{	State_21++;		}	//Mcl1aBak
	if (current_Receptor->state == 4)	{	State_4++;		}	//Bak (bound to mitochondria)
	if (current_Receptor->state == 41)	{	State_41++;		}	//aBak
	if (current_Receptor->state == 42)	{	State_42++;		}	//aBak2
	if (current_Receptor->state == 44)	{	State_44++;		}	//aBak4
	if (current_Receptor->state == 46)	{	State_46++;		}	//aBak6
	if (current_Receptor->state == 56)	{	State_56++;		}	//tBidMito
	if (current_Receptor->state == 51)	{	State_51++;		}	//tBidBak
	if (current_Receptor->state == 52)	{	State_52++;		}	//tBidaBak
	if (current_Receptor->state == 54)	{	State_54++;		}	//tBidMcl1
...
//	collision_counter_ligand_total += current_Ligand->collision_counter_ligand;
    ...
    if (current_Ligand->state == 2)		{	State_2++;		}	//Mcl1Cyto
	//if (current_Receptor->state == 4)	{	State_4++;		}	//Bak
	if (current_Receptor->state == 5)	{	State_5++;		}	//tBid
...
file = fopen(excel_filepath, "a");
    fprintf(file, "%f\t", itno*0.0050000);	/* has to be changed to reaction time */
    fprintf(file, "%d\t", State_2);
    fprintf(file, "%d\t", State_20);
	fprintf(file, "%d\t", State_21);
	fprintf(file, "%d\t", State_4);
    fprintf(file, "%d\t", State_41);
	fprintf(file, "%d\t", State_42);
	fprintf(file, "%d\t", State_44);
	fprintf(file, "%d\t", State_46);
	fprintf(file, "%d\t", State_5);
    fprintf(file, "%d\t", State_56);
	fprintf(file, "%d\t", State_51);
	fprintf(file, "%d\t", State_52);
	fprintf(file, "%d\t", State_54);
